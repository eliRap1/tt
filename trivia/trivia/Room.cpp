...
void Room::removeUser(const LoggedUser& user) { m_users.erase(std::remove(m_users.begin(), m_users.end(), user), m_users.end()); }
const std::vector<std::string> Room::getUsers() const { return std::vector<std::string>(m_users.begin(), m_users.end()); }
